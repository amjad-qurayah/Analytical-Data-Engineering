-- Getting Last Date
-- setting the variable LAST_SOLD_WK_SK to the maximum SOLD_WK_SK from the WEEKLY_SALES_INVENTORY table.
...
--  delete any records from WEEKLY_SALES_INVENTORY table where the sold_wk_sk matches the value of LAST_SOLD_WK_SK
...
-- A temporary table WEEKLY_SALES_INVENTORY_TMP is created. 
-- This table is populated by aggregating daily sales data to the week level,
--  correcting any missing Sunday sales dates, and obtaining additional inventory data for each item.
...
-- then joins the aggregated sales data with the inventory data using week number and year number, grouping the results by relevant fields.
...
-- the  metrics are calculated based on the joined data.
select 
       warehouse_sk, 
       item_sk, 
